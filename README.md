...
**Run any command, browse your files, and drive coding CLIs like Claude, Codex & Cursor — from your phone, from anywhere.** End-to-end encrypted, direct peer-to-peer. No account to try.
...
PhoneCMD turns your phone into a real terminal for the computer you already own. Not a toy — a **persistent shell** that stays alive between commands, so `claude`, `codex`, `cursor`, a REPL, a long `npm run build` — all keep going while you check on them from bed, a café, or a train.
...
- 🤖 **Coding CLIs from your phone.** It detects Claude, Codex, Copilot, Cursor, Gemini, Antigravity and Aider, and launches them in a live session you can watch and answer — pick a folder, pass extra flags, or send a first prompt.
- ↩️ **Resume past conversations.** For Claude, Codex, Copilot, Cursor and Antigravity, PhoneCMD reads the CLI's own on-disk sessions and lets you **reopen the exact chat by id** — from this folder, or browse **every project on the PC** and resume any one. Each new tab is its own separate conversation.
...
- 🔒 **Truly end-to-end encrypted.** Remote traffic goes **direct peer-to-peer** — anyone on the wire sees only ciphertext, and it never passes through our servers. Your machine's keys never leave it.
- 🌍 **Works anywhere.** Same WiFi → direct LAN. Different network → direct WebRTC peer-to-peer. The app picks automatically; you do nothing.

## Get started in 60 seconds

1. **[Download the installer](../../releases/latest)** for your OS. Nothing else to install.
...
📱 Phone (or the web client)  ──E2E──▶  ws:// LAN direct     ─┐
                              crypto    (same WiFi)          ├─▶  🖥️ THIS APP
                                        WebRTC P2P (remote)  ─┘     runs your commands
                                         (direct; never via our servers)
```

The phone is a thin client; **your PC does the work.** Commands and files travel
phone → (LAN or direct P2P) → this app, encrypted the whole way. A lightweight
directory service just tells your phone where the PC is right now, and brokers a
brief WebRTC handshake so the two peers find each other — it never sees a single
command or byte of your files.

### Security, in plain terms

- **Nobody can read your traffic** — X25519 handshake + `crypto_box` encryption, end to end, direct peer-to-peer. Anyone on the wire sees ciphertext only.
...
npm run cli -- serve                   # start a host + print the QR + keycode
```
